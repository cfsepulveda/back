...
# Funciones para la carga de datos
    ...
    def create_index_order(videos, *args, self):
        order_indexes = self.names_to_indexes(args)
        index_order = model.create_index_order(videos, order_indexes)
        return index_order

    def most_days_tendency(videos, index_order, selection_parameters, print_parameters, self):
        floor = 0
        ...
        if days > maxdays:
            maxdays = days
        print_indexes = self.names_to_indexes(print_parameters)
    ...
    def top_videos_order(videos, index_order, selection_parameters, print_parameters, n=None, tag = None, top=True, self):
        floor = 0
        ceiling = len(index_order) - 1
        if selection_parameters:
            floor, ceiling = model.range_by_parameter(videos, index_order, selection_parameters)
